...
/**
 * Register photos ALREADY in the S3 bucket as an event's gallery, so face
 ...
 * copies. GalleryService::delete() only removes S3 objects living inside the
...
class ImportGalleryPhotos extends Command
{
    ...
    protected $description = 'Import existing S3 photos into a NATCON event gallery and index their faces';
    ...
    public function handle(FaceRecognitionService $faces): int
    {
        ...
        $existing = GalleryPhoto::whereIn('s3_key', $images)->pluck('s3_key')->all();
