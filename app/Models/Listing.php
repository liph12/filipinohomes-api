...
class Listing extends Model
{
    ...
    public function scopeFilter(Builder $query, Request $request): Builder
    {
        ...
        if ($request->filled('beds')) {
            ...
            $query->whereHas('property.propertyAttribute', fn($q) => $q->where(
                'bedroom_count',
                match ($bedsCondition) { 'plus' => '>=', 'minus' => '<=', default => '=' },
                $beds
            ));
        }
        ...
        if ($request->filled('baths')) {
            ...
            $query->whereHas('property.propertyAttribute', fn($q) => $q->where(
                'bathroom_count',
                match ($bathsCondition) { 'plus' => '>=', 'minus' => '<=', default => '=' },
                $baths
            ));
        }
    ...
    private function applySqmSort(Builder $query, string $sortBy): Builder
    {
        ...
        return $query
            ->orderByRaw(
                "GREATEST(COALESCE(property_attributes.lot_area,0), COALESCE(property_attributes.floor_area,0)) {$direction}"
            )
            ->select('listings.*');
    }
