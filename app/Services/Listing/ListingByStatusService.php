...
class ListingByStatusService extends ListingInsightsService
{
    ...
    public function listingsForStatus(string $status, array $params, ?array $agentIds = null): array
    {
        ...
        $sortMap = [
            ...
            'audited' => 'listings.audited_at',
            'submitted' => DB::raw('COALESCE(listings.re_submitted_at, listings.created_at)'),
        ...
        $rows = (clone $base)
            ...
            ->select(
                ...
                DB::raw('(SELECT t.name FROM team_agents ta JOIN teams t ON t.id = ta.team_id WHERE ta.agent_id = listings.agent_id AND ta.status = \'active\' ORDER BY ta.id LIMIT 1) as team_name'),
